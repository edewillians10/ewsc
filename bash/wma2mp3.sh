...
#
# PURPOSE: Convert wma files specified on the command line to mp3 using
...
audiodump=audiodump.wav
trap 'rm -f $audiodump; exit 1' 1 2 3 15

for i in "$@"; do
    n=$(echo $i | sed -e "s/\.wma$/.mp3/")
    $mplayer -vo null -af resample=44100 -ao pcm "$i" && $converter -m s $bitrate_op $audiodump "$n"
done
